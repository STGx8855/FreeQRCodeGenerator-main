/* public/assets/js/wifi.js
   Page logic for wifi.html
   - Validate SSID (required)
   - Handle encryption selection (hide password for nopass)
   - 3s placeholder animation + QR creation
   - optional logo processing
...
(function () {
  ...
  function validateForm() {
    clearInvalids();
    const ssidEl = qs("#wifi-ssid");
    const enc = (qs("#wifi-encryption") && qs("#wifi-encryption").value) || "WPA";
    const pwdEl = qs("#wifi-password");

    if (!ssidEl || ssidEl.value.trim().length === 0) {
      markInvalid(ssidEl, "Wi-Fi name (SSID) cannot be empty");
      return false;
    }

    if (enc !== "nopass") {
      // require a password for WPA/WEP
      if (!pwdEl || pwdEl.value.trim().length === 0) {
        markInvalid(pwdEl, "Password required for selected encryption");
        return false;
      }
    }
  ...
  function buildData() {
    ...
    const ssid = (qs("#wifi-ssid").value || "").replace(/([;:\\,"])/g, '\\$1'); // escape ;,:\"\
    const enc = qs("#wifi-encryption").value;
    const pwd = (qs("#wifi-password").value || "");
  ...
  // render helper (same pattern used elsewhere)
  function renderQRToDOM(instance) {
    ...
    try {
      ...
      console.warn("Could not clear wifi form:", e);
    }
  }
  ...
  async function generateCore() {
    ...
    // hook download/share
    qs("#btn-download").onclick = () => window.QRApp.downloadPNG(qrInstance, "qr-wifi");
  ...
  // show/hide password depending on encryption
  function setupEncryptionToggle() {
    const enc = qs("#wifi-encryption");
    const pwdSec = qs("#passwordSection");
    if (!enc || !pwdSec) return;
    enc.addEventListener("change", () => {
      if (enc.value === "nopass") pwdSec.style.display = "none";
      else pwdSec.style.display = "";
    });
    // initial state
    if (enc.value === "nopass") pwdSec.style.display = "none";
  }
